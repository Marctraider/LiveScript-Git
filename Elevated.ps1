...
$model = (gwmi Win32_ComputerSystem).Model; if ( $model -like 'Blade Stealth 13 (Early 2020) - RZ09-0310') {
    ...
    New-ItemProperty -Path $Path -Name "Start" -PropertyType DWord -Value 4 -Force
    $Path = "HKLM:\SYSTEM\CurrentControlSet\Services\igfxCUIService2.0.0.0"; if(-not (Test-Path -Path $Path)){ New-Item -ItemType String -Path $Path }
    New-ItemProperty -Path $Path -Name "Start" -PropertyType DWord -Value 4 -Force
    }
...
New-ItemProperty -LiteralPath 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Svchost' -Name 'netsvcs' -Value @("CertPropSvc","SCPolicySvc","lanmanserver","gpsvc","IKEEXT","iphlpsvc","seclogon","msiscsi","EapHost","schedule","winmgmt","ProfSvc","SessionEnv","wercplsupport","PushToInstall","InstallService","LxpSvc","shpamsvc","XblGameSave","DmEnrollmentSvc","WManSvc","TokenBroker","lfsvc","FastUserSwitchingCompatibility","Ias","Irmon","Nla","Ntmssvc","NWCWorkstation","Nwsapagent","Rasauto","Rasman","Remoteaccess","SENS","Sharedaccess","SRService","Tapisrv","Wmi","WmdmPmSp","wuauserv","ShellHWDetection","LogonHours","PCAudit","helpsvc","uploadmgr","dmwappushservice","wisvc","WpnService","wlidsvc","XboxNetApiSvc","NetSetupSvc","NaturalAuthentication","Themes","DsmSvc","NcaSvc","AppInfo","XboxGipSvc","XblAuthManager","UserManager","AppMgmt","BDESVC") -PropertyType MultiString -Force -ea SilentlyContinue;
...
New-NetFirewallRule -Display 'Script Generated: Redundant' -Direction Outbound -Protocol Any -RemoteAddress 89.39.105.32 -Action Allow
...
New-NetFirewallRule -Display 'Script Generated: NCSI' -Direction Outbound -Action Allow -Program 'C:\Windows\System32\svchost.exe' -Service 'NlaSvc'
...
Set-NetAdapterBinding -Name '*' -DisplayName 'QoS Packet Scheduler' -Enabled 1 -ea SilentlyContinue
...
$model = (gwmi Win32_ComputerSystem).Model; if ( $model -like 'MS-7B12') {
    ...
    #Set-NetAdapterAdvancedProperty -Name '*Ethernet*' -DisplayName 'Maximum Number of RSS Queues' -RegistryValue '2'
...
# Disable all system-wide exploit mitigations except DEP, and except Windows default exclusions.
...
New-ItemProperty -Path $Path -Name "MitigationOptions" -PropertyType Binary -Value ([byte[]](0x20,0x22,0x22,0x00,0x00,0x02,0x00,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00)) -Force
...
$model = (gwmi Win32_ComputerSystem).Model; if ( $model -like 'MS-7B12') {
    # Custom DPI
    $Path = "HKU:\Control Panel\Desktop"; if(-not (Test-Path -Path $Path)){ New-Item -ItemType String -Path $Path }
    New-ItemProperty -Path $Path -Name "LogPixels" -PropertyType Dword -Value 114 -Force

    # Enable Enhance Mouse Pointer
    $Path = "HKU:\Control Panel\Mouse"; if(-not (Test-Path -Path $Path)){ New-Item -ItemType String -Path $Path }
    ...
    $Path = "HKU:\Control Panel\Mouse"; if(-not (Test-Path -Path $Path)){ New-Item -ItemType String -Path $Path }
    New-ItemProperty -Path $Path -Name "SmoothMouseXCurve" -PropertyType Binary -Value ([byte[]](0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x30,0x33,0x0F,0x00,0x00,0x00,0x00,0x00,0x60,0x66,0x1E,0x00,0x00,0x00,0x00,0x00,0x90,0x99,0x2D,0x00,0x00,0x00,0x00,0x00,0xC0,0xCC,0x3C,0x00,0x00,0x00,0x00,0x00)) -Force
...
-and $_.Taskname -notmatch 'Sysprep Generalize Drivers' -and $_.Taskname -notmatch 'Device Install Group Policy' -and $_.Taskname -notmatch 'ResPriStaticDbSync' -and $_.Taskname -notmatch 'WsSwapAssessmentTask' `
 -and $_.Taskname -notmatch 'DXGIAdapterCache' -and $_.Taskname -notmatch 'UninstallDeviceTask' -and $_.Taskname -notmatch 'ExploitGuard MDM policy Refresh'} | Disable-ScheduledTask
...
-and $_.PSChildName -NotLike 'UBPM' -and $_.PSChildName -NotLike 'NetCore' -and $_.PSChildName -NotLike 'LwtNetLog' }
